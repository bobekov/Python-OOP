...
class BankApp:
    ...
    def get_statistics(self):
        ...
        result.append(f"Granted Loans: {granted_loans:.2f}")

        granted_amount = sum([sum([loan.amount for loan in c.loans]) for c in self.clients])
        result.append(f"Total Sum: {granted_amount:.2f}")

        result.append(f"Available Loans: {len(self.loans)}")

        sum_not_granted = sum([l.amount for l in self.loans])
        result.append(f"Total Sum: {sum_not_granted:.2f}")
